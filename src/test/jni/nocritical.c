...
Java_io_nano_time_jni_NativeTimeNoCritical_currentTimeMicros(JNIEnv *env, jobject thisObj) {
...
JNIEXPORT jlong JNICALL
Java_io_nano_time_jni_NativeTimeNoCritical_currentTimeNanos(JNIEnv *env, jobject thisObj) {
#if defined(_POSIX_TIMERS)
  {
    struct timespec ts;
    if ( 0 == clock_gettime(CLOCK_REALTIME, &ts) )
